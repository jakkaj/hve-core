...
This phase creates the educational foundation for the entire site: four content pages under "Getting Started" that teach what HyperVelocity Engineering is, how HVE-Core's architecture works, and where each user persona should go first. It also creates the `docusaurus-edits.instructions.md` file that ensures all future content follows Docusaurus conventions automatically.

### What We're Building

Five "Getting Started" pages plus one instructions file:

* `intro.md` — "What is HyperVelocity Engineering?" with value delivery loop diagram, coverage heatmap, DORA/SPACE/ESSP summary
